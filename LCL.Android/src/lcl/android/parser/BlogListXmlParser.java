...
public class BlogListXmlParser extends DefaultHandler {
	...
	private ArrayList<BlogEntity> listBlog;// 对象集合
	private BlogEntity entity;// 单个对象
	 ...
	 * @return
	 */
	public BlogListXmlParser(ArrayList<BlogEntity> list) {
		this.listBlog = list;
	}
	...
	public void startDocument() throws SAXException {
		Log.i("Blog", "文档解析开始");
		super.startDocument();
		listBlog = new ArrayList<BlogEntity>();
	...
	public void endElement(String uri, String localName, String qName)
		...
		if (isStartParse) {// 发现目标
			...
			if (localName.equalsIgnoreCase(ENTRY_TITLE_TAG)) {// 标题
				try {
					chars = HtmlRegexpUtil.replaceTag(chars);// 进行编码处理，避免出现&gt;这种html
				} catch (Exception ex) {
				...
				Date addTime = StringUtils.ParseUTCDate(chars);
				entity.SetAddTime(addTime);
			} else if (localName.equalsIgnoreCase(ENTRY_UPDATED_TAG)) {// 修改时间
				Date updateTime = StringUtils.ParseUTCDate(chars);
