...
public class BlogDalHelper {
	...
	private boolean Exist(int blogId) {
		...
		Cursor cursor = db.query(AppConfig.DB_BLOG_TABLE, null, where, args,
				null, null, null);
	...
	private boolean IsFull(int blogId) {
		...
		boolean isFull = cursor.getString(cursor.getColumnIndex("IsFull"))
				.equals("1");
	 ...
	 * @return
	 */
	public List<BlogEntity> GetTopBlogList() {
		String limit = "10";
		String where = "";
	 ...
	 * 分页
	 */
	public List<BlogEntity> GetBlogListByPage(int pageIndex, int pageSize) {
		String limit = String.valueOf((pageIndex - 1) * pageSize) + ","
				+ String.valueOf(pageSize);
		List<BlogEntity> list = GetBlogListByWhere(limit, null, null);
	 ...
	 * @return
	 */
	public List<BlogEntity> GetBlogListByAuthor(String author, int pageIndex,
			int pageSize) {
		String limit = String.valueOf((pageIndex - 1) * pageSize) + ","
				+ String.valueOf(pageSize);
		String where = "AuthorName=?";
		String[] args = { author };
		List<BlogEntity> list = GetBlogListByWhere(limit, where, args);
	 ...
	 * 得到对象
	 */
	public BlogEntity GetBlogEntity(int blogId) {
		String limit = "1";
		String where = "BlogId=?";
		String[] args = { String.valueOf(blogId) };
		List<BlogEntity> list = GetBlogListByWhere(limit, where, args);
	 ...
	 * @param where
	 */
	public List<BlogEntity> GetBlogListByWhere(String limit, String where,
			String[] args) {
		List<BlogEntity> listBlog = new ArrayList<BlogEntity>();
		...
		while (cursor != null && cursor.moveToNext()) {
			...
			Date addTime = StringUtils.ParseDate(addTimeStr);
			...
			if (cursor.getString(cursor.getColumnIndex("Updated")) != null) {
				updateTime = StringUtils.ParseDate(cursor.getString(cursor
	...
	public boolean GetIsReaded(int blogId) {
		BlogEntity entity = GetBlogEntity(blogId);
		if (entity != null) {
	...
	public void MarkAsReaded(int blogId) {
		String sql = "update " + AppConfig.DB_BLOG_TABLE
	...
	public void SynchronyContent2DB(int blogId, String blogContent) {
		...
		String sql = "update " + AppConfig.DB_BLOG_TABLE
				+ " set Content=?,IsFull=1 where BlogId=?";
	 ...
	 * @param list
	 */
	public void SynchronyData2DB(List<BlogEntity> blogList) {
		...
		for (int i = 0, len = blogList.size(); i < len; i++) {
			...
			contentValues
					.put("Published", StringUtils.ParseDateToString(blogList
							.get(i).GetAddTime()));
			Date datetime = new java.util.Date();
			String updateTime = "";
			if (blogList.get(i).GetUpdateTime() != null) {
				updateTime = StringUtils.ParseDateToString(blogList.get(i)
						.GetUpdateTime());
			} else {
				updateTime = StringUtils.ParseDateToString(datetime);
			}
		...
		synchronized (_writeLock) {
			...
			try {
				...
				for (int i = 0, len = list.size(); i < len; i++) {
					...
					if (!isExist) {
						db.insert(AppConfig.DB_BLOG_TABLE, null, list.get(i));
